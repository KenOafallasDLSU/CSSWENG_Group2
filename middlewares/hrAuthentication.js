...
const hrAuthentication = {
    ...
    isValidHR: (req, res, next)=>{        
        if(req.params.sUsername == req.session.userId){
            db.findOne(modelSREP, {sUsername: req.session.userId, bHRStatus: true}, '', function(objSREP){
                if (objSREP){
                    next();
                }
                else{
...
module.exports = hrAuthentication;